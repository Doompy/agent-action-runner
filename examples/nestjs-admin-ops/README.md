# NestJS Admin Ops Example

This example shows the same operational mutation flow as the Express example, but through NestJS DI and `@AgentAction()` discovery:
...
```bash
npm install
npm run build --workspace agent-action-runner-nestjs-admin-ops-example
node examples/nestjs-admin-ops/dist/index.js
```
...
```bash
curl -s http://localhost:3001/agent-runner/actions/admin.searchUsers/execute \
...
```bash
curl -s http://localhost:3001/agent-runner/actions/admin.dryRunDisableUser/execute \
  -H "content-type: application/json" \
  -H "x-user-id: operator_1" \
...
```bash
curl -s http://localhost:3001/approvals/disable-user \
  -H "content-type: application/json" \
  -H "x-user-id: operator_1" \
...
```bash
curl -s http://localhost:3001/agent-runner/actions/admin.disableUser/execute \
  -H "content-type: application/json" \
  -H "x-user-id: operator_1" \
...
```bash
curl -s http://localhost:3001/audit
```
